...
- Framework-agnostic Chart.js engine: chart lifecycle (mount, diffed
  ...
  labels, gradient, timestack, hierarchical, image label, and
  autocolors, and inline/custom Chart.js plugin passthrough
...
- `ChartConfiguration`, `ChartJs`, `ChartKind`, `ChartUpdatePayload`,
  ...
  `ImageLabelPluginOptions`, `AutocolorsPluginOptions`
...
- Single generic `<Chart type="...">` component for Vue 3 covering all 15
  chart kinds, all 8 official plugins, and inline custom plugins.
...
- `zoom`, `annotation`, `dataLabels`, `gradient`, `timestack`,
  `hierarchical`, `imageLabel`, `autocolors` opt-in props, plus a
  `plugins` prop for any custom or community plugin outside those 8.
...
- 46 component tests, 100% coverage, 97.73% mutation score.
- 78/78 Playwright e2e tests passing across Chromium, Firefox, and WebKit.
...
- Astro + Starlight documentation site with full Vue guide, API, components,
  ...
  chart events, colors plugin, zoom plugin, gradient plugin, image label
  plugin, hierarchical scale, and autocolors plugin — all live and
  interactive; sankey and timestack-scale source-only pending a
  docs-site build-pipeline gap).
