...
describe('imageLabelPlugin', () => {
  ...
  it('evicts the oldest cached image once the cache reaches its cap, so a later draw of that URL re-loads rather than reuses it', async () => {
    ...
    //
    // The original version of this test only checked that a later draw
    // of the evicted URL calls ctx.drawImage once — true whether it was
    // a fresh load OR a still-cached hit, since drawImage is called
    // exactly once either way. The real, observable difference is
    // WHEN: a cache hit calls ctx.drawImage synchronously (see the
    // "caches a loaded image" test above), a fresh load only calls it
    // after awaiting the image's own onload. Checking synchronously,
    // before any await, is what actually distinguishes "genuinely
    // evicted, reloading" from "never evicted, still cached" — and
    // doing the same for the SECOND url inserted (which real behavior
    // never evicts, since only the very first one crosses the cap)
    // catches an inverted or always-true condition that evicts on every
    // insert instead of only once.
    const firstUrl = `evict-first-${Math.random()}.png`;
    const secondUrl = `evict-second-${Math.random()}.png`;
    for (let i = 0; i < 201; i++) {
      const url = i === 0 ? firstUrl : i === 1 ? secondUrl : `evict-filler-${i}-${Math.random()}.png`;
      const ctx = makeCtx();
      const chart = {
        ctx,
        data: { datasets: [{}] },
    ...
    // firstUrl crossed the 200-entry cap and should have been evicted —
    ...
    // secondUrl never crossed the cap on its own and should still be
