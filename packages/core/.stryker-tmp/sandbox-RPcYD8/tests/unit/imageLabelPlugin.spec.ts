...
describe('imageLabelPlugin', () => {
  ...
  it('evicts the oldest cached image once the cache reaches its cap, so a later draw of that URL re-loads rather than reuses it', async () => {
    ...
    // instead, making the exact eviction math directly verifiable with
    // far fewer, more reliable operations.
    vi.resetModules();
    ...
    // fill-1 is one entry newer than the evicted fill-0 and should
    // still be cached — a draw of it must hit synchronously.
