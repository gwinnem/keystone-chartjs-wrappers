...
describe('imageLabelPlugin', () => {
  ...
  it('evicts the oldest cached image once the cache reaches its cap, so a later draw of that URL re-loads rather than reuses it', async () => {
    ...
    // public afterDraw API by drawing distinct URLs, the same way a
    // real long-running page cycling through many images would.
    //
    // loadedImages is a module-level singleton shared across every test
    // in this file (never reset), so its size going into this test isn't
    // 0 — an earlier version of this test assumed exactly 201 inserts
    // would evict only the very first one, which broke depending on how
    // many prior tests had already cached their own distinct URLs.
    // "Flushing" with a full MAX_CACHED_IMAGES(200) batch of fillers
    // first guarantees the cache is ENTIRELY this batch afterward,
    // regardless of its starting size (size can never exceed 200, so
    // 200 more inserts are always enough to cycle out anything older).
    // From that known baseline, firstUrl/secondUrl land at precise,
    // predictable ages, and exactly 199 further fillers evict firstUrl
    // specifically while leaving secondUrl untouched.
    //
    // The real, observable difference between "evicted, reloading" and
    // "still cached, hit" is WHEN ctx.drawImage is called: a cache hit
    // calls it synchronously (see the "caches a loaded image" test
    // above), a fresh load only calls it after awaiting the image's own
    // onload — checking synchronously, before any await, is what
    // actually distinguishes the two (an earlier version of this test
    // only checked call COUNT, true either way since drawImage is
    // called exactly once regardless).
    ...
    for (let i = 0; i < 200; i++) {
      await draw(`evict-flush-${i}-${Math.random()}.png`);
    }

    const firstUrl = `evict-first-${Math.random()}.png`;
    const secondUrl = `evict-second-${Math.random()}.png`;
    await draw(firstUrl);
    await draw(secondUrl);

    for (let i = 0; i < 199; i++) {
      await draw(`evict-filler-${i}-${Math.random()}.png`);
    }

    // firstUrl is now the oldest entry and should have been evicted —
    // a fresh draw must NOT hit synchronously; it needs its own load.
    ...
    imageLabelPlugin.afterDraw(firstChart as never, {}, { imagesList: [{ imageUrl: firstUrl, imageWidth: 10, imageHeight: 10 }] });
    expect(firstCtx.drawImage).not.toHaveBeenCalled();
    await flushImageLoad();
    expect(firstCtx.drawImage).toHaveBeenCalledTimes(1);

    // secondUrl is one entry newer than firstUrl and should still be
    // cached — a draw of it must hit synchronously, no load needed.
    ...
    imageLabelPlugin.afterDraw(secondChart as never, {}, { imagesList: [{ imageUrl: secondUrl, imageWidth: 10, imageHeight: 10 }] });
    expect(secondCtx.drawImage).toHaveBeenCalledTimes(1);
  });
