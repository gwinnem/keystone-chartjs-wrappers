...
describe('zoomPlugin', () => {
  ...
  describe('precise mutation-hardening: zoom/pan math (zoomDelta, log-scale, fixRange, updateRange, category)', () => {
    ...
    it('updateRange: rejects a pan whose min OR max alone would violate a limit (kills || → && on the pan-limit check)', () => {
      ...
      // range.max > maxLimit)` — an && mutation on the inner `||` would
      // require BOTH sides to violate their own limit simultaneously. A
      // pan whose own max alone exceeds maxLimit, with min staying
      // comfortably within bounds, isolates the `range.max > maxLimit`
      // side specifically.
      const scale = makeScale({ id: 'x', min: 80, max: 95 });
      const chart = makeChart({ scales: { x: scale } });
      initPlugin(chart, { pan: { enabled: true }, limits: { x: { min: 0, max: 100 } } });

      // A pan of +20 would push max to 115 (over the 100 limit) while
      // min (100) stays within [0,100] — real: rejected outright (early
      // return, scale.options never written at all). If the `||` were
      // mutated to `&&`, this single-sided violation wouldn't trigger
      // the early rejection, and the pan would instead proceed to
      // fixRange, which would clamp differently (writing scale.options).
      chart.pan({ x: -20, y: 0 });
