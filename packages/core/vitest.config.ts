...
export default defineConfig({
  test: {
    ...
    coverage: {
      ...
      // file, not source).
      include: ['src/**/*.ts'],
      // types.ts and chartjs-plugin-trendline.d.ts are excluded
      // deliberately, not to dodge the floor: both are 100% type-only
      // declarations (interfaces/type aliases; an ambient module
      // declaration for chartjs-plugin-trendline, which ships no types
      // of its own), which TS erases entirely at compile time — there
      // is no runtime statement for v8 to ever instrument, so either can
      // only ever report 0%, which is a tooling artifact, not untested
      // logic. Every other file under src/ has real runtime code and
      // stays in scope.
      exclude: ['src/types.ts', 'src/chartjs-plugin-trendline.d.ts'],
